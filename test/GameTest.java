...
public class GameTest {
    ...
    public void testRun() {
        ...
        view.setModel(model);
        assertEquals(view.getModel(), (Model)model);
        assertEquals(control.getModel(), (Model)model);
        // Start the game view, which may initialize the UI or display the starting screen
        view.start();
        SimpleIO simpleIO = new SimpleIO();
        simpleIO.setTimeOut(1);
        simpleIO.runInput(model);
        assertTrue(model.isTimeOut());
        ...
        assertFalse(r);
        CineGame cineGame = new CineGame();
        String input = "p1\np2\n";
