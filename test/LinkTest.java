...
public class LinkTest {
    ...
    public void testConnectionOK() {
        MovieData movieData = new MovieData();
        Movie movie = movieData.getMovieByTitle("mission: impossible");
        Movie movie1 = movieData.getMovieByTitle("red 2");
        Movie movie2 = movieData.getMovieByTitle("Eyes Wide Shut");
        Movie movie3 = movieData.getMovieByTitle("minority report");
        Movie movie4 = movieData.getMovieByTitle("Edge of Tomorrow");
        Movie movie5 = movieData.getMovieByTitle("The Bourne Identity");
        Movie movie6 = movieData.getMovieByTitle("The Bourne ultimatum");
        Links links = new Links();
        links.setCurrentMovie(movie);
        assertTrue(links.addLink(movie1)); // same connection type
        assertFalse(links.addLink(movie2)); // same connection type
        assertTrue(links.addLink(movie3)); // same connection type
        assertTrue(links.addLink(movie4)); // same connection type
        assertTrue(links.addLink(movie5)); // same connection type
        assertTrue(links.addLink(movie6)); // same connection type
        assertTrue(links.getCommonGenre() != null);
        assertEquals(links.getCurrentMovie(), movie6);
        assertEquals(links.getLink(0).getMovie1(), movie);
    }

    @Test
    public void testGenre(){
        Genre genre = new Genre("test", 1);
